...
class OficinaRequest extends FormRequest
{
    ...
    {
        $oficId = $this->oficina ? $this->oficina->ofic_id : null;

        return [
            'ofic_nombre' => 'required|string|max:200|unique:oficinas,ofic_nombre,' . $oficId . ',ofic_id',
            'ofic_descripcion' => 'nullable|string',
            'ofic_ubigeo' => 'nullable|string|size:6',
            'ofic_direccion' => 'nullable|string',
            'ofic_siglas' => 'required|string|max:15|unique:oficinas,ofic_siglas,' . $oficId . ',ofic_id',
