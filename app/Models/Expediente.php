...
class Expediente extends Model
{
    ...
    protected function getExpeEditarAttribute()
    {
        $tramites = Tramite::selectRaw('tram_esta_id , COUNT(tram_esta_id) as count')->where('tram_expe_id', $this->expe_id)->groupBy('tram_esta_id')->get();
        $count = $tramites->count();
        if ($count === 1) {
            if ($tramites[0]->tram_esta_id === 5) {
                return true;
            } else {
                return false;
            }
    ...
    public static  $headersExpe =  [
        ['text' => "Tramite", 'value' => "expe_proc_id", 'short' => false, 'order' => 'ASC'],
        ['text' => "Asunto", 'value' => "expe_asunto", 'short' => false, 'order' => 'ASC'],
        ['text' => "Resumen", 'value' => "expe_resumen", 'short' => false, 'order' => 'ASC'],
        ['text' => "Remitente", 'value' => "expe_pers_id", 'short' => false, 'order' => 'ASC'],
        ['text' => "Estado", 'value' => "expe_esta_id", 'short' => false, 'order' => 'ASC'],
        ['text' => "Fecha", 'value' => "expe_fecha_registro", 'short' => false, 'order' => 'ASC'],
    ];

    public static function getExpedientes(Request $request, $tipo,  Admin $admin = null)
    {
        ...
        $items = $query
            ...
            ->where('expe_admin_id', $admin->id)
            ->whereIn('expe_tipo', $tipo)
            ->with(['persona', 'estado', 'proceso'])
            ->orderBy('expe_id', 'desc')
            ->paginate($perPage)->appends($request->query());

        return [
            'items' => $items,
            'headers' => self::$headersExpe,
            'filters' => [
    ...
    public static function getPendientes(Request $request, Admin $admin = null)
    {
        ...
        $items = $query
            ...
            ->where('expe_esta_id', 2) //en tramite
            ->whereIn('tram_esta_id', [5, 6]) //PENDIENTE, //RECIBIDO
    ...
    public static function getArchivados(Request $request, Admin $admin = null)
    {
        $perPage = $request->input('perPage', 10);
        $query = self::query();
        ...
        $items = $query
            ...
            ->where('tram_admin_fin', $admin->id)
            ->where('expe_esta_id', 2) //en tramite
            ->where('tram_esta_id', 9) //PENDIENTE, //RECIBIDO
            ->with(['persona', 'estado', 'proceso', 'documento'])
            ->paginate($perPage)->appends($request->query());
    ...
    public static function getObservados(Request $request, Admin $admin = null)
    {
        $perPage = $request->input('perPage', 10);
        $query = self::query();
        ...
        $items = $query
            ...
            ->join('tramites', 'tram_expe_id', 'expe_id')
            ->where('tram_admin_fin', $admin->id)
            ->where('expe_esta_id', 3) //en tramite ---OBSRVADOS
            ->where('tram_esta_id', 10) //Observados
            ->with(['persona', 'estado', 'proceso', 'documento'])
            ->paginate($perPage)->appends($request->query());
    ...
    public static function getFinalizados(Request $request, Admin $admin = null)
    {
        $perPage = $request->input('perPage', 10);
        $query = self::query();
        ...
        $items = $query
            ...
            ->join('tramites', 'tram_expe_id', 'expe_id')
            ->where('tram_admin_fin', $admin->id)
            ->where('expe_esta_id', 4) //en finalizado
            ->where('tram_esta_id', 8) //Observados
            ->with(['persona', 'estado', 'proceso', 'documento'])
            ->paginate($perPage)->appends($request->query());
    ...
    public static function getDerivados(Request $request, Admin $admin = null)
    {
        $perPage = $request->input('perPage', 10);
        $query = self::query();
        ...
        $items = $query
            ...
            ->where('tram_admin_fin', $admin->id)
            ->where('expe_esta_id', 2) //en tramite
            ->where('tram_esta_id', 7) //Observados
            ->with(['persona', 'estado', 'proceso', 'documento'])
            ->paginate($perPage)->appends($request->query());
